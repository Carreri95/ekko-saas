...
} from "@/lib/openai-api-key-storage";
...
export default function GerarPage() {
  ...
  const handleConfigRun = useCallback(
    async (config: ModalConfig) => {
      ...
      const openaiKeyHeader = openAiKey.trim();
    ...
    [modalMode, retryJobId, batchId, modalViewOnly, openAiKey],
  );
        ...
        >
          <div className="batch-gen-toolbar">
            <div className="batch-gen-toolbar-openai">
              <label
                className="batch-gen-openai-strip-label"
                htmlFor={openAiKeyId}
              >
                Chave OpenAI
              </label>
              <input
                ...
                placeholder="sk-… (só nesta sessão)"
                title="A chave fica só nesta sessão do navegador (sessionStorage). É enviada ao servidor apenas ao iniciar ou repetir transcrições."
              />
              <a
                className="batch-gen-openai-strip-link"
