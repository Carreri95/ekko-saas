...
/**
 * O import de `ffmpeg-static` no Next/Turbopack pode devolver um caminho virtual (`\\ROOT\\...`)
 * que não existe no disco — tentamos primeiro caminhos reais sob `process.cwd()`.
 */
function ffmpegPath(): string {
  ...
  const fromPkg = typeof ffmpegStatic === "string" ? ffmpegStatic.trim() : "";
  if (fromPkg) {
  ...
  throw new Error(
    "ffmpeg não encontrado. Corra `npm install` na pasta web, ou instale o ffmpeg no sistema e defina FFMPEG_BIN com o caminho completo do executável.",
  );
}
