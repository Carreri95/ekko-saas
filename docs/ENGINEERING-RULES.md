...
abstrações “talvez úteis no futuro”

3. REGRA DE NOVAS ROTAS
   3.1 Regra geral
...
inventar fetch manual duplicado quando já existe helper adequado

4. REGRA DE CONTRATO PÚBLICO
...
reindexação
mensagens de erro
criação de versionamento

7. REGRA DE STORAGE E ARQUIVOS
...
aplicar retry técnico quando aplicável
...
banco
export
smoke de editor quando fizer sentido
...
Aplicar migrações (desenvolvimento local com alteração ao schema)
npm run db:migrate

Faz: `prisma migrate dev` em `apps/api` — cria e aplica uma **nova** migração quando alteras `apps/api/prisma/schema.prisma`. **Não** é o fluxo típico de CI que só aplica ficheiros já commitados.
...
Faz: regenera o client do Prisma (API e web via generator `client_web`). Obrigatório após pull com mudanças em schema/migrations antes de build, quando aplicável.

Deploy de migrações (apenas aplicar migrações versionadas)
npm run db:deploy

Faz: `prisma migrate deploy` em `apps/api` — aplica migrações pendentes sem prompts. **Uso típico:** CI, staging, produção, BD nova ou clone sem alterar o schema. Falha se o histórico em `_prisma_migrations` não for compatível com o repositório.

Para cenários (BD nova, dev com histórico antigo, staging, `migrate resolve`, inspecção de `_prisma_migrations`), ver **`docs/monorepo-operacao.md`** — secção _Prisma (pós-consolidação): operação segura_.
...
13.4 Ordem recomendada (arranque local habitual)
...
sobe banco
aplica migrações (em dev com possível alteração ao schema; ver 13.5)
popula base
sobe API
sobe web
sobe worker

13.5 Prisma: migrate vs deploy (resumo)

- **Primeiro clone / BD vazia / só aplicar o que está no repo:** preferir `db:up` → `db:deploy` → `db:generate` → `db:seed` (e depois apps), em linha com `docs/monorepo-operacao.md`.
- **Alteração ao schema em desenvolvimento:** `db:migrate`.
- **CI, staging, produção:** `db:deploy` — **não** usar `db:migrate` só para “aplicar” migrações já commitadas.

14. VARIÁVEIS IMPORTANTES
...
ressalvas foram registradas com honestidade

16. TEMPLATE DE PEDIDO PARA O CURSOR
...
**TEMPLATE DE FECHAMENTO DOCUMENTAL**
...
18. REGRAS COMPLEMENTARES DE MONOREPO

18.1 Prefixo oficial de rotas HTTP

Salvo decisão explícita em contrário, toda rota HTTP pública do produto deve usar prefixo `/api/...`.
...
- evitar ambiguidade entre rota de negócio e página do Next
- manter o BFF do apps/web simples
- alinhar apps/api e apps/web no mesmo path observável

  18.2 Prisma: fonte de verdade vs pipeline operacional

O schema Prisma de negócio é de responsabilidade do apps/api.

Se houver cópia/espelho de Prisma em apps/web por motivos operacionais
(migrate, seed, generate, tooling), isso deve obedecer às regras:

- deve existir uma fonte de verdade explícita
- deve existir um diretório autoritativo de migrations explícito
- o espelho não pode divergir semanticamente da fonte de verdade
- não criar migrations concorrentes em apps/api e apps/web sem decisão explícita

**Fonte de verdade actual (monorepo):**

- Schema: `apps/api/prisma/schema.prisma` (único ficheiro de modelos; inclui segundo generator `client_web` para o client usado pelo Next).
- Migrations: `apps/api/prisma/migrations/` (único directório de histórico).
- Comandos oficiais na **raiz**: `npm run db:migrate`, `npm run db:deploy`, `npm run db:generate` (delegam para `apps/api`). O `apps/web` mantém `prisma.config.ts` a referenciar o schema e as migrations da API para `db seed` e tooling; não criar novas pastas de migração em `apps/web/prisma/migrations`.
- Procedimentos operacionais (BD nova, dev com histórico antigo, staging, `migrate resolve`, inspecção de `_prisma_migrations`): **`docs/monorepo-operacao.md`** — secção _Prisma (pós-consolidação): operação segura_.

Toda alteração de Prisma deve dizer claramente:

- onde o schema autoritativo vive
- onde a migration autoritativa vive
- qual comando oficial aplica a mudança

  18.3 Estados oficiais de validação

Toda validação deve declarar explicitamente em qual estado cada evidência se encontra:

- IMPLEMENTADO: existe em código
- VALIDADO EM EXECUÇÃO: testado de verdade com serviços no ar
- VALIDADO POR INSPEÇÃO: inferido por leitura de código/contrato
- NÃO VALIDADO: não foi executado nem confirmado
...
quando API/web/worker/banco necessários para a prova não estiverem realmente acessíveis.

18.4 Auth via BFF e cookies

Quando houver autenticação baseada em cookie:

- o browser deve falar com o apps/web em same-origin (`/api/...`)
- o BFF deve repassar `Cookie` ao apps/api quando aplicável
- o BFF deve preservar `Set-Cookie` vindo do apps/api
- a lógica de sessão continua sendo do apps/api
- o apps/web não é dono da autenticação; ele só transporta e reflete estado

Em desenvolvimento local com HTTP:

- cookies `Secure` não devem bloquear o fluxo em localhost
- variáveis de ambiente e comportamento esperado devem estar documentados

  18.5 Efeitos externos e padrão outbox

Integrações externas com efeito fora do processo
(ex.: email, webhook, notificações, dispatch para provider)
devem preferir o padrão:

- apps/api registra a intenção de envio de forma transacional
- apps/worker faz o envio assíncrono
- o estado do dispatch deve ser rastreável
- falhas devem ser registradas honestamente

Evitar, salvo motivo explícito:

- envio direto pelo apps/web
- envio síncrono em request crítica do apps/api
- side effect não rastreável

  18.6 Ajustes incidentais fora do escopo

Se um PR precisar incluir correções incidentais fora do escopo principal
para compilar, buildar ou destravar o slice:

- devem ser mínimas
- devem ser sem mudança semântica intencional
- devem ser declaradas explicitamente como incidentais
- se forem maiores que isso, devem ir para outro PR
