...
import {
  getCueProblems,
} from "@/app/(private)/subtitle-file-edit/lib/cue-problems";
import {
  autoBrText,
  computeCueCps,
  CPS_WARN_ABOVE,
  ...
  toSaveCuePayload,
  validateCuesForSave,
} from "@/app/(private)/subtitle-file-edit/lib/cue-utils";
import { formatPlaybackTime } from "@/app/(private)/subtitle-file-edit/lib/format-time";
import {
  scrollCueIntoListPanel,
} from "@/app/(private)/subtitle-file-edit/lib/dom-utils";
import type {
  AspectRatio,
  CueDto,
  LocalWaveformData,
...
function SubtitleFileEditPageInner() {
  ...
  const [projectHeader, setProjectHeader] = useState<
    { name: string; status: DubbingProjectStatus } | null
  >(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(`/api/dubbing-projects/${encodeURIComponent(pid)}`);
        if (!res.ok) return;
        const data = (await res.json()) as {
          project?: { name?: string; status?: DubbingProjectStatus };
        };
        const proj = data.project;
        if (!cancelled && proj?.name && proj?.status) {
          setProjectHeader({ name: proj.name, status: proj.status });
        }
      } catch {
        // Sem impacto: breadcrumb/badge do header vão cair no fallback "—".
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    return () => {
      if (redirectAfterSaveTimerRef.current) {
  ...
  const handleDeleteCueFromContext = useCallback(
    async (cue: CueDto) => {
      const ok = await confirm({
        title: "Eliminar cue",
        description: `Confirma eliminar a cue #${cue.cueIndex}? Pode desfazer com o histórico do editor (Ctrl+Z) logo a seguir.`,
        variant: "danger",
  ...
  const handleSaveAndComplete = useCallback(async () => {
    ...
    try {
      ...
      // Notificação existente no editor: `saveSuccess` renderiza o "toast/notification" via UI.
      setSaveSuccess("SRT salvo com sucesso");
      // Permite o utilizador ver a mensagem antes de retornar à lista de episódios.
      redirectAfterSaveTimerRef.current = window.setTimeout(() => {
        router.push(`/projetos/${encodeURIComponent(pid)}?tab=episodios`);
      }, 2000);
    } finally {
      ...
      <div className="editor-desktop-shell flex min-h-0 min-w-0 flex-1 flex-col gap-1 overflow-hidden">
        ...
        {dubbingContextActive ? (
          <DubbingEpisodeNavBar
            ...
            showEditorToolbar={hasEditorContent || loading}
            editorFilename={filename}
            editorCuesCount={cues.length}
            editorWavFilename={wavFilename}
            editorLoading={loading}
            editorSaving={saving}
            editorExporting={exporting}
            editorSaveSuccess={saveSuccess}
            editorError={error}
            projectId={projectIdParam.trim()}
            projectName={projectHeader?.name ?? null}
            projectStatus={projectHeader?.status ?? null}
          />
        ) : null}
        {!dubbingContextActive && (hasEditorContent || loading) ? (
