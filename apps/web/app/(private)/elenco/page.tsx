...
export default function ElencoPage() {
  ...
  const deleteMember = useCallback(
    async (m: CastMemberDto) => {
      const ok = await confirm({
        title: "Eliminar dublador",
        description: `Confirma eliminar "${m.name}" do elenco? Esta ação não pode ser desfeita.`,
        variant: "danger",
        confirmLabel: "Sim, eliminar",
      });
      if (!ok) return;
      const res = await fetch(`/api/cast-members/${encodeURIComponent(m.id)}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) {
        ...
        window.alert(data.error ?? "Não foi possível eliminar o dublador.");
        return;
      }
      void fetchMembers();
    },
    [confirm, fetchMembers],
  );
