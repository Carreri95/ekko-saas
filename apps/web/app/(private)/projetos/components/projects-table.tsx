...
type Props = {
  ...
  onOpenNew?: () => void;
  /** Eliminar projeto (DELETE `/api/dubbing-projects/:id`); confirmar na página. */
  onDeleteProject?: (p: DubbingProjectDto) => void | Promise<void>;
};
            ...
            <tr key={p.id} onClick={() => onRowClick(p)}>
              ...
              <td onClick={(e) => e.stopPropagation()}>
                <div className="row-actions">
                  {onDeleteProject ? (
                    <TableRowActionsMenu
                      ariaLabel={`Ações do projeto ${p.name}`}
                    >
                      <TableRowMenuDangerItem
                        withBorderTop={false}
