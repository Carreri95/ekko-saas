...
import { getAudioDurationMsFromBuffer } from "../projects/audio-duration.js";
import type { MediaStorageService } from "../projects/media-storage.service.js";
import type { TranscriptionJobService } from "../transcription-jobs/transcription-job.service.js";

/**
 * Subconjunto de `apps/web` BatchJobService: create + status + start (PR 6.3.1),
 * upload de ficheiro no batch (PR 6.3.2), remoção de jobs do lote (DELETE batch jobs),
 * download ZIP (PR 6.3.5 / último slice batch-jobs).
 */
export class BatchJobService {
  ...
  async processBatchSequential(
    ...
    const pendingCount = await this.prisma.transcriptionJob.count({
    ...
    if (pendingCount === 0) {
      await this.prisma.batchJob.update({
        where: { id: batchId },
        data: { status: BatchJobStatus.DONE, completedAt: new Date() },
      });
      return;
    }
  ...
  async addFileFromUpload(params: {
    ...
    const durationMs = await getAudioDurationMsFromBuffer(params.buffer, params.mimeType);
  ...
  /**
   * Remove jobs do lote (só PENDING ou FAILED). Apaga projeto, ficheiro em disco e cues (cascade).
   * Portado de `apps/web` BatchJobService.removeJobsFromBatch.
   */
  ...
  /**
   * ZIP com SRT dos jobs DONE (exportFormat SRT). Portado de `apps/web` BatchJobService.buildZipForDoneJobs.
   */
  async buildZipForDoneJobs(batchId: string): Promise<Buffer> {
    ...
    const { formatSrt } = await import("../projects/srt/format-srt.js");
